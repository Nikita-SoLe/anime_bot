...
from handlers.touch_main_button import press_ova_btn
...
@router.callback_query(OVA_State.anime, Text('back_to_genre'))
async def touch_back_to_genre_btn(callback: CallbackQuery):
    await callback.message.delete()
    await press_ova_btn(*users_db[callback.from_user.id]['state'])
